...
let bindTuple p1 p2 state =
    match p1 state with
    | Ok (r, s) -> match p2 s with
                   | Ok (r2, s) -> Ok ([(r, r2)], s)
                   | Error -> Error 
    | Error -> Error

let bindLeft p1 p2 state =
    match bindTuple p1 p2 state with
    | Ok (r, s) -> Ok (Seq.head r |> fst, s)
    | Error -> Error 

let bindRight p1 p2 state =
    match bindTuple p1 p2 state with
    | Ok (r, s) -> Ok (Seq.head r |> snd, s)
    | Error -> Error
...
let until p =
    fun stream ->
        let rec doUntil acc stream =
            match p stream with
            | Ok (_, s) -> Ok(List.rev acc, s)
            | Error -> let c, s = next stream
                       doUntil (c.Value :: acc) s
        doUntil [] stream
